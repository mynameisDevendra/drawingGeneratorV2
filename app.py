        ...
        for group in parts[1:]:
            ...
            if match:
                ...
                for i in range(start, end + 1):
                    new_rows.append({"Row ID": rid, "Header": h_text, "Footer": "", "Terminal ID": str(i).zfill(2)})
        return new_rows
    except: return None
    ...
    for rid, group in rows:
        ...
        for key, is_h, y_off in [('Header', True, 53.5), ('Footer', False, -13.5)]:
            ...
            while i < len(terms):
                ...
                c.setLineWidth(0.8); c.line(s_x-5, y_current+y_off, e_x+5, y_current+y_off); mid = (s_x+e_x)/2
                c.setFont("Helvetica-Bold", fs['head' if is_h else 'foot'])
                if is_h:
                    c.line(s_x-5, y_current+y_off, s_x-5, y_current+y_off-5); c.line(e_x+5, y_current+y_off, e_x+5, y_current+y_off-5)
...
st.set_page_config(page_title="Railway Terminal Tool", layout="wide")
st.title("🚉 Railway Terminal Professional Designer")

if 'df' not in st.session_state: st.session_state.df = pd.DataFrame(columns=["Row ID", "Header", "Footer", "Terminal ID"])

with st.sidebar:
    with st.expander("🛠️ Layout", expanded=True):
...
st.subheader("Fast Data Input")
nlp_input = st.text_input("Format: A row, Label [1 to 5], Label [6 to 10]")
if st.button("Add Group"):
    parsed = parse_fixed_format(nlp_input)
    if parsed: st.session_state.df = pd.concat([st.session_state.df, pd.DataFrame(parsed)], ignore_index=True)
st.session_state.df = st.data_editor(st.session_state.df, num_rows="dynamic", use_container_width=True)
if st.button("🗑️ Clear Table"): st.session_state.df = pd.DataFrame(columns=["Row ID", "Header", "Footer", "Terminal ID"]); st.rerun()
